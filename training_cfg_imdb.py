...
import random
import numpy as np
dataset_org = load_from_disk("imdb")
...
def add_mask(x):
    ...
    x["text"] = x["text"] + "<counterfactual>" + sent
    x = tokenizer(x["text"])
    
    percent = random.uniform(0.05,0.50)
...
for epoch in range(num_epochs):
    ...
    shuffled_data["unsupervised"] = shuffled_data["unsupervised"].select(range(100))
    ...
    torch.save({
            ...
            }, f"counter_{model_id}_imdb.pt")

# %%
if model_id != "pythia":
    model.merge_and_unload()
    torch.save(model.base_model.model.state_dict(),
                f"counter_{model_id}_imdb_finished.pt")
