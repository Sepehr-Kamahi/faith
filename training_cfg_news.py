...
dataset_org = load_from_disk("news")

dataset_org = dataset_org.filter(lambda x:x["label"] in [0,1,2,3])
dataset_org["test"] = dataset_org["test"].select(range(3000))
...
if model_id == "pythia2":
    ...
    peft_config = LoraConfig(task_type=TaskType.CAUSAL_LM, inference_mode=False,target_modules = target_modules, r=128, lora_alpha=256, lora_dropout=0.1)
    model = get_peft_model(model_base, peft_config)
    model.print_trainable_parameters()
elif model_id == "gptj":
...
def add_mask(x):
    ...
    if x["label"] == 0:
        x["sentence"] = sent + " world"
    elif x["label"] == 1:
        x["sentence"] = sent + " sport"
    elif x["label"] == 2:
        x["sentence"] = sent + " business"
    elif x["label"] == 3:
        x["sentence"] = sent + " tech"  

    length = len(tokenizer(sent)["input_ids"])

    x["sentence3"] = x["sentence"] + "<counterfactual>" + sent
    x = tokenizer(x["sentence3"])
...
def data_processing(shuffled_data):
    ...
    data = shuffled_data.filter(lambda x:len(tokenizer(x["text"])["input_ids"])<=max_seq_length//2 - 1)
...
for epoch in range(num_epochs):
    ...
    model.eval()
    with torch.no_grad():
        for i in tqdm(range(len(data["test"])//batch_size)):
            ids = torch.LongTensor(data["test"][i*batch_size:(i+1)*batch_size]["input_ids"]).to(device)
            mask = torch.LongTensor(data["test"][i*batch_size:(i+1)*batch_size]["attention_mask"]).to(device)
            labels = torch.LongTensor(data["test"][i*batch_size:(i+1)*batch_size]["it_labels"]).to(device)
    ...
    torch.save({
            ...
            }, f"counter_{model_id}_news.pt")

# %%
if model_id != "pythia":
    model.merge_and_unload()
    torch.save(model.base_model.model.state_dict(),
                f"counter_{model_id}_news_finished.pt")
