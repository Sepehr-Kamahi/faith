# This code was adapted from the repository https://github.com/kayoyin/interpret-lm by Kayo Yin.
...
def saliency(model, input_ids, input_mask, batch=0, correct=None, foil=None):
    ...
    model.zero_grad()
    if model.config.model_type == 'opt' or model.config.model_type == 'bloom' or model.config.model_type == 'llama' or model.config.model_type=="phi3" or model.config.model_type == "gemma":
    ...
    if foil is not None and correct != foil:
        ...
        if model.config.model_type == 'opt' or model.config.model_type == 'bloom':
...
def input_x_gradient(grads, embds, normalize=False):
    ...
    if normalize:
        #print('before input_grad', input_grad)
        norm = np.linalg.norm(input_grad, ord=1)
        input_grad /= norm
        #print('after input_grad', input_grad)
...
def l2_grad_norm(grads, normalize=False):
    l1_grad = np.linalg.norm(grads, ord=2, axis=-1).squeeze()
